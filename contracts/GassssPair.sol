...
contract GassssPair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function _balanceOf(address token, address owner) private returns (uint) {
        (bool success, bytes memory data) = token.staticcall(abi.encodeWithSelector(BALANCE_OF_SELECTOR, owner));
        if (!success) {
            (success, data) = token.call(abi.encodeWithSelector(BALANCE_OF_SELECTOR, owner));
        }
        require(success && data.length >= 32, 'UniswapV2: BALANCE_FAILED');
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        uint balance0 = _balanceOf(token0, address(this));
        uint balance1 = _balanceOf(token1, address(this));
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        uint balance0 = _balanceOf(_token0, address(this));
        uint balance1 = _balanceOf(_token1, address(this));
    ...
    function skim(address to) external lock {
        ...
        _safeTransfer(_token1, to, _balanceOf(_token1, address(this)).sub(reserve1));
    }
